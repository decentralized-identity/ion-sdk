import { DidKey, IonPublicKeyPurpose, JwkEd25519 } from '../lib/index';
...
describe('DidKey', async () => {
  ...
  describe('generateEs256kDidDocumentKeyPair()', async () => {
    ...
    it('should throw error if given DID Document key ID exceeds maximum length.', async () => {
      ...
      await JasmineIonErrorValidator.expectIonErrorToBeThrownAsync(
        async () => DidKey.generateEs256kDidDocumentKeyPair({ id, purposes: [IonPublicKeyPurpose.Authentication] }),
        ErrorCode.IdTooLong
      );
    });
    ...
    it('should throw error if given DID Document key has duplicated purposes.', async () => {
      await JasmineIonErrorValidator.expectIonErrorToBeThrownAsync(
        async () => DidKey.generateEs256kDidDocumentKeyPair({ id: 'anyId', purposes: [IonPublicKeyPurpose.Authentication, IonPublicKeyPurpose.Authentication] }),
  ...
  describe('generateEd25519DidDocumentKeyPair()', async () => {
    ...
    it('should throw error if given DID Document key ID exceeds maximum length.', async () => {
      ...
      await JasmineIonErrorValidator.expectIonErrorToBeThrownAsync(
        async () => DidKey.generateEd25519DidDocumentKeyPair({ id, purposes: [IonPublicKeyPurpose.Authentication] }),
        ErrorCode.IdTooLong
      );
    });
    ...
    it('should throw error if given DID Document key has duplicated purposes.', async () => {
      await JasmineIonErrorValidator.expectIonErrorToBeThrownAsync(
        async () => DidKey.generateEd25519DidDocumentKeyPair({ id: 'anyId', purposes: [IonPublicKeyPurpose.Authentication, IonPublicKeyPurpose.Authentication] }),
        ErrorCode.PublicKeyPurposeDuplicated
      );
    });
  });
